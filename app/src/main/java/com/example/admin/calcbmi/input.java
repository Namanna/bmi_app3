...
public class input extends AppCompatActivity {
    ...
    public void btnClick(View v) {
        ...
        if (editText.length()==0 && editText2.length()==0) {
            Toast.makeText(this, "Hãy nhập cân nặng và chiều cao!", Toast.LENGTH_SHORT).show();

        }
        else if (editText.length() == 0)
        {
            Toast.makeText(this, "Hãy nhập số cân!", Toast.LENGTH_SHORT).show();
        }
        else if(editText2.length() == 0)
        {
            Toast.makeText(this, "Hãy nhập chiều cao!", Toast.LENGTH_SHORT).show();
        }
        else
            {
